...
class AI85UartAdapter(AI85Adapter):
    ...
    def get_network_out(self, data):
        ...
        for j in range(batch):
          ...
          #  data_batch = data_batch - data_batch
          #  data_batch += 1
            sum += np.sum(data_batch)
            print(f'batch shape: {data_batch.shape} sum: {hex(sum)}')
            #self.__transmit_data(data_batch)
            self.__transmit_data_crc(data_batch)
            print(f'batch={j+1}')
            time.sleep(.1)
    ...
    def __release_device(self):
        ...
        while True:
            ...
            if sync_message == final_device_message:
                ...
                print(f'RX from HW: {reading.decode()}')
    ...
    def __transmit_data_crc(self, data):
        temp = np.int8(data - 128)
        byte_arr = temp.copy()
        byte_arr[:, :, 0] = temp[:, :, 2]
        byte_arr[:, :, 2] = temp[:, :, 0]
        ...
        for i_0 in range(shape_0):
             for i_1 in range(shape_1):
                 ...
                 cnt += 3
                 reading = self.ser.readline()
                 print(f'\r{cnt} {reading[0:len(reading)-1].decode()}',end="")
